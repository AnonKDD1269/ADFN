...
class FuncControlModel(nn.Module):
    ...
    def forward(self, x, debug_flag=False):
        deb_flag = False
        out_res = []
        self.mid_outputs = []
        if debug_flag == True:
        ...
        for i in range(self.n_func):
            if i == 0:
                out = x 
            out = self.layers[i](out, self.alphas[i], deb_flag)
    ...
    def get_top_func_names_layer(self,topidx):
        ...
        flist = [dir(self.flist_inst)[i] for i in range(len(dir(self.flist_inst))) if not dir(self.flist_inst)[i].startswith('_')]

        top_lists = [flist[i] for i in idx_list]
