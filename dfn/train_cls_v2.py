...
import math 
from model import FuncControlModel, RefModel, RefConvModel
import copy
# Define your model
import random 
...
def train(args, model=None, ref_model=None):
    ...
    # mnist_validset
    val_set = torchvision.datasets.MNIST(root='./data', train=False, download=True, transform=transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,)), transforms.Resize([28,28])]))
    val_loader = torch.utils.data.DataLoader(val_set, batch_size=bs, shuffle=False, drop_last=True)


    if args.rounds == 0:
        model = FuncControlModel(args.layer, in_length =dataset.in_length,cls_num=dataset.cls_num)
    else: 
    ...
    if args.mode =='solo':
        ...
        for epoch in pbar:
            ...
            for i, data in pbar_inner:
                ...
                appr_loss = appr_criterion(outputs, ref_output) 
                total_loss = loss + appr_loss
...
def ref_model_training(args):
    ...
    if os.path.exists(f'./result_model/ref_model_{args.type}_{args.fig_name}.pth'):
        model.load_state_dict(torch.load(f'./result_model/ref_model_{args.type}_{args.fig_name}.pth'))
    ...
    for epoch in pbar:
        ...
        for images, labels in tqdm(train_loader):
            ...
            if correct/batch_size > max_acc:
                max_acc = correct/batch_size
            running_loss += loss.item()
            pbar.set_postfix({'max_acc': max_acc ,
                              'loss': running_loss/len(train_loader), 
                              'acc': correct/batch_size})

    torch.save(model.state_dict(), f'./result_model/ref_model_{args.type}_{args.fig_name}.pth')
